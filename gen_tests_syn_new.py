...
def pred_sent_by_syn(pred_list, masked_temp, words, round, pre_score):
    tests_set = []
    new_temps = []
    score_list = []
    ...
    for i in range(len(pred_list)):
        ...
        if "[MASK]" in mask_sent:
            ...
            if new_sent not in tests_set:
                tests_set.append(new_sent)
                new_temps.append(format_abbr(masked_temp[i]))
                score_list.append(0.5 * pre_score)
    ...
    for i in range(len(comp_list)):
        ...
        for j in range(len(masked_adjunct_list)):
            ...
            if j == 0:
                ...
                new_tests, new_temps, score_list = pred_sent_by_syn(pred_list, masked_temp, words, j, 1)
                ...
                for t in range(len(next_temp_list)):
                    ...
                    new_tests, new_temps, score_list = pred_sent_by_syn(pred_list, masked_temp, words, j, old_score_list[t])
